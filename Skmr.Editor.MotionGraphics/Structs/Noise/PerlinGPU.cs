...
namespace Skmr.Editor.MotionGraphics.Structs.Noise
{
   ...
   138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180};
            ...
            double smoothedX = SmoothToSCurve(x);
            double smoothedY = SmoothToSCurve(y);
            double smoothedZ = SmoothToSCurve(z);
            ...
            result[idx.X, idx.Y] = (float)LinearlyInterpolate(ZZeroPlaneVal, ZOnePlaneVal, smoothedZ);
        }
        ...
        private static double SmoothToSCurve(double val)
        {
            return val * val * val * (val * (val * 6d - 15d) + 10d);
        }

        private static double Dot(Vec3D gradient, double x, double y, double z)
        {
